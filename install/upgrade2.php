    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                if (!$db->fieldExists($table, 'sex')) {
                    $db->query("ALTER TABLE `$table` ADD `sex` VARCHAR(1)");
                }
                if (!$db->fieldExists($table, 'phone')) {
                    $db->query("ALTER TABLE `$table` ADD `phone` VARCHAR(32)");
                }
                if (!$db->fieldExists($table, 'fax')) {
                    $db->query("ALTER TABLE `$table` ADD `fax` VARCHAR(32)");
                }
                if (!$db->fieldExists($table, 'email')) {
                    $db->query("ALTER TABLE `$table` ADD `email` VARCHAR(255) DEFAULT NULL");
                }
                if (!$db->fieldExists($table, 'website')) {
                    $db->query("ALTER TABLE `$table` ADD `website` VARCHAR(255) DEFAULT NULL");
                }
                if (!$db->fieldExists($table, 'type')) {
                    $db->query("ALTER TABLE `$table` ADD `type` TINYINT(1)");
                }
                if (!$db->fieldExists($table, 'sex')) {
                    $db->query("ALTER TABLE `$table` ADD `sex` VARCHAR(1)");
                }
                if (!$db->fieldExists($table, 'id_card')) {
                    $db->query("ALTER TABLE `$table` ADD `id_card` VARCHAR(13) DEFAULT NULL");
                }
                if (!$db->fieldExists($table, 'address')) {
                    $db->query("ALTER TABLE `$table` ADD `address` VARCHAR(150) DEFAULT NULL");
                }
                if (!$db->fieldExists($table, 'provinceID')) {
                    $db->query("ALTER TABLE `$table` ADD `provinceID` VARCHAR(3) DEFAULT NULL");
                }
                if (!$db->fieldExists($table, 'province')) {
                    $db->query("ALTER TABLE `$table` ADD `province` VARCHAR(50) DEFAULT NULL");
                }
                if (!$db->fieldExists($table, 'zipcode')) {
                    $db->query("ALTER TABLE `$table` ADD `zipcode` VARCHAR(10) DEFAULT NULL");
                }
                if (!$db->fieldExists($table, 'country')) {
                    $db->query("ALTER TABLE `$table` ADD `country` VARCHAR(2) DEFAULT 'TH'");
                }
                ...
                if (!$db->tableExists($table)) {
                    ...
                    $sql .= "`th` text COLLATE utf8_unicode_ci DEFAULT NULL,";
                    $sql .= "`en` text COLLATE utf8_unicode_ci DEFAULT NULL,";
